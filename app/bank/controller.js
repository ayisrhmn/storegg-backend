...
module.exports = {
  index: async (req, res) => {
    try {
      ...
      res.render('bank/bank', {bank, alert});
    } catch (err) {
  ...
  actionCreate: async (req, res) => {
    try {
      const {accName, bankName, noRek} = req.body;

      let bank = await Bank({accName, bankName, noRek});
      await bank.save();
  ...
  actionEdit: async (req, res) => {
    try {
      ...
      await Bank.findOneAndUpdate({_id: id}, {accName, bankName, noRek});
